...
function formatDate(dateString: string): string {
  ...
  function getWeek(dateString: string): string {
    // Calculate weeks since August 17, 2026 (first day of fall semester)
...
export function scrollToAnchor(
  ...
  // Check if container can actually scroll (CSS alone is not enough — a flex
  // child without min-h-0 can report overflow:auto while not overflowing).
  const canScrollContainer =
    isElementScrollable(scrollContainer) &&
...
const SITE_BASE_PATH = '/fall2026';

export function withBasePath(url?: string): string | undefined {
  if (!url) return url;
  if (url.startsWith('/') && !url.startsWith(`${SITE_BASE_PATH}/`) && url !== SITE_BASE_PATH) {
