...
function assignmentTitleShort(type?: string, num?: string | number): string {
  const n = num ?? '';
  if (type === 'homework') return `HW ${n}`;
  if (type === 'lab') return `Lab ${n}`;
  if (type === 'project') return `Project ${n}`;
  if (type === 'tutorial') return `Tutorial ${n}`;
  if (type === 'exam') return `Exam ${n}`;
  return n !== '' ? `${type ? type : 'Assignment'} ${n}` : (type || 'Assignment');
}
...
function normalizeDate(dateStr: string | Date | undefined): string | null {
...
async function enrichTopicsWithMarkdown(baseTopics: BaseTopicsArray): Promise<TopicsArray> {
  ...
  const allExams = getAllPosts('exams');
  ...
  const examsWithDates = allExams.filter(e => e.due_date);
  ...
  const examsByDate = new Map<string, PostData[]>();
  ...
  examsWithDates.forEach(exam => {
    const date = normalizeDate(exam.due_date);
    if (date) {
  ...
  enrichedTopics.forEach((topic) => {
    topic.meetings.forEach((meeting) => {
      ...
      const matchingExams = examsByDate.get(meetingDateStr) || [];
      ...
      const autoExams = matchingExams.map((exam: PostData) => ({
        title: exam.title,
        url: `/exams/${exam.id}/`,
        draft: exam.draft || 0,
        excluded: exam.excluded ? 1 : 0,
        order: exam.ordering ?? exam.order,
      }));
      autoActivities.push(...autoExams);
      ...
      const autoAssignedAssignments = matchingAssignmentsByAssigned.map((assignment) => {
        const titleShort = assignmentTitleShort(assignment.type, assignment.num);
        return {
  ...
  if (orphanedAssignments.size > 0) {
    ...
    orphanedAssignments.forEach((assignments, dateStr) => {
      ...
      if (targetTopic) {
        ...
        const isLab = assignments.some(a => a.type === 'lab');
        const isProject = assignments.some(a => a.type === 'project');
        let topicName = 'Assignment';
        if (isLab && !isHomework && !isTutorial && !isProject) {
          topicName = 'Lab';
        } else if (isProject && !isHomework && !isTutorial && !isLab) {
          topicName = 'Project';
